...
const getValoraciones = async (req, res) => {
  try {
    ...
    const allValoraciones = await Reserva.find()
      .populate("experiencia")
      .populate("usuario"); // Hemos comentado la linea anterior para que no pinte toda la información (por ejemplo el "__v"). En esta linea, después del populate("experiencias"), le estamos diciendo que nos pinte los campos "experiencia fecha estado total". Esto se escribe sin coma).
